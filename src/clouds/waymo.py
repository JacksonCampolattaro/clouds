...
def _convert_frames(args):
    raw_path, test_ids = args
    raw_split_dir, raw_filename = os.path.split(raw_path)
    raw_dir, split = os.path.split(raw_split_dir)
    processed_dir = os.path.join(os.path.split(raw_dir)[0], 'processed')
    processed_filename = os.path.join(split, raw_filename.replace('.tfrecord', '.pt'))
    ...
    frame_data_list = []
    for raw_record in tfrecord_iterator(raw_path):
        frame = Frame()
        frame.ParseFromString(raw_record)

        # Skip unlabeled frames, unless they're in the test set
        if (
            not len(frame.lasers[0].ri_return1.segmentation_label_compressed)
            and f'{frame.context.name},{frame.timestamp_micros}' not in test_ids
        ):
            continue

        frame_data_list.append(_frame_to_data(frame))

    if not frame_data_list:
        return []

    # Save all frames from this TFRecord as a single .pt file
    assert frame_data_list
    data, slices, _ = collate(Data, frame_data_list, increment=False, add_batch=False)
    torch.save((slices, data), os.path.join(processed_dir, processed_filename))

    # Return the list of names & indices
    return [(processed_filename, i) for i in range(len(frame_data_list))]
...
class SemanticWaymo(Dataset):
    ...
    def get(self, idx: int) -> BaseData:
        frames_file, frame_idx = self._data[idx]
        # Load the collated data for this TFRecord
        slices, collated_data = torch.load(os.path.join(self.processed_dir, frames_file), weights_only=False)
        return separate(
            cls=Data,
            batch=collated_data,
            idx=frame_idx,
    ...
    def process(self) -> None:
        ...
        for split in split_ids:
            ...
            # Create processed split directory
            os.makedirs(os.path.join(self.processed_dir, split), exist_ok=True)
            
            # Glob all tfrecord files in this split
            filenames = glob.glob(os.path.join(self.raw_dir, split, '*.tfrecord'))
            with (
                Progress(*Progress.get_default_columns(), progress.TimeElapsedColumn()) as p,
                multiprocessing.Pool(processes=8) as pool,
            ):
                task = p.add_task(f"Loading Waymo {split} data", total=len(filenames))
                p.update(task, advance=0)
                for result in pool.imap_unordered(_convert_frames, [(f, test_frame_ids) for f in filenames]):
                    split_ids[split].extend(result)
                    p.update(task, advance=1)

        # Save slices for each filename
