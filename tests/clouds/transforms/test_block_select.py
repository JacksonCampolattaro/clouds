...
class TestBlockSelect:
    ...
    def test_forward_basic_data_custom_params(self, basic_data):
        ...
        transform = BlockSelect(max_num_points=20, min_num_points=5, selection_factor=0.8)
        result = transform(basic_data)
    ...
    def test_forward_batched_data(self, batched_data):
        ...
        expected_indices = torch.cat([torch.arange(0, 25), torch.arange(50, 65), torch.arange(80, 90)])
    ...
    def test_forward_batched_data_clamp(self, batched_data):
        ...
        transform = BlockSelect(max_num_points=12, min_num_points=5, selection_factor=0.8)
        result = transform(batched_data)

        # Expected: 40, 24, 16 -> clamped to 12, 12, 12
        expected_sizes = [12, 12, 12]
        expected_indices = torch.cat([torch.arange(0, 12), torch.arange(50, 62), torch.arange(80, 92)])
        assert torch.all(result.selection_index == expected_indices)
    ...
    def test_forward_batched_data_min_clamp(self, batched_data):
        ...
        expected_indices = torch.cat([torch.arange(0, 10), torch.arange(50, 60), torch.arange(80, 90)])
        assert torch.all(result.selection_index == expected_indices)
    ...
    def test_forward_selection_factor_zero(self, basic_data):
        ...
        transform = BlockSelect(selection_factor=0.0, min_num_points=10, max_num_points=100)
        result = transform(basic_data)
    ...
    def test_repr(self):
        ...
        transform = BlockSelect(max_num_points=1000, selection_factor=0.5)
    ...
    def test_large_number_points(self):
        ...
        transform = BlockSelect(max_num_points=5000, selection_factor=0.8)
    ...
    def test_parametrized_basic_data(self, basic_data, selection_factor, min_points, max_points, expected_size):
        ...
        transform = BlockSelect(selection_factor=selection_factor, min_num_points=min_points, max_num_points=max_points)
        result = transform(basic_data)
        assert len(result.selection_index) == expected_size
